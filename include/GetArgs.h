...
// This is a helper/utility class to convert an arbitrary string into argc/argv format
...
{
public:
    const char** getArgs(const char* str, uint64_t& argc)
    {
        argc = 0;
...
private:
    ...
    inline const char* skipSpaces(const char* str) const
    {
        if (str)
        {
            while (*str == 32 || *str == 9)
